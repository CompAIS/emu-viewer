...
class ContourWidget(BaseWidget):
    ...
    def __init__(self, root):
        ...
        parameters_label.grid(column=0, row=0, padx=10, pady=10, sticky=tk.W)

        # Parameters
        mean_label = tb.Label(self.frame, text="Mean", bootstyle="inverse-light")
        mean_label.grid(column=0, row=1, padx=10, pady=10, sticky=tk.W)

        mean_entry = tb.Entry(self.frame)
        mean_entry.grid(column=1, row=1, padx=10, pady=10)

        sigma_label = tb.Label(self.frame, text="Sigma", bootstyle="inverse-light")
        sigma_label.grid(column=0, row=2, padx=10, pady=10, sticky=tk.W)
        ...
        sigmas_label.grid(column=0, row=3, padx=10, pady=10, sticky=tk.W)

        sigmas_entry = tb.Entry(self.frame)
        sigmas_entry.grid(column=1, row=3, padx=10, pady=10)
...
# Parameter X label in X+1, 1
# Parameter X input in X+1, 2

# Sigma List label in 5, 0
# Sigma List entry in 5, 1->end-1

# Generate Levels button in 6, 0

# Levels label in 7, 0
# Level list entry in 7, 1->end-1

# Apply button in end, end
# Close button in end, end
